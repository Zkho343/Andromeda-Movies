...
def parse_movie_list_request(movies_per_page: int, request: request) -> Tuple[List[Movie], str, str, str, str]:
    ...
    movies = services.get_num_movies(movies_per_page, offset, repo.repo_instance)
    total_movies = services.get_movie_number(repo.repo_instance)
...
def parse_movie_search_request(movies_per_page: int, request: request, form: MovieSearchForm,
    ...
    search_func_map = {
        'Actor': services.get_movies_actor,

        'Director': services.get_movies_director,

        'Genre': services.get_movies_genre,
